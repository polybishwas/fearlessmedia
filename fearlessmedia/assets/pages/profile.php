...
<button class="btn btn-sm btn-danger unfollowbtn" data-user-id='<?=$profile['id']?>' >Unfollow</button>
   ...
   <?php
    ...
    ?>
<button class="btn btn-sm btn-success followbtn" data-user-id='<?=$profile['id']?>' >Follow</button>
...
?>
        ...
        <h3 class="border-bottom"><i class="bi bi-file-image"></i>Posts</h3>
        <?php
...
if(checkBS($profile['id'])){
 ...
 <div class="alert alert-secondary text-center" role="alert">
    <i class="bi bi-x-circle-fill"></i> When you block someone they will no longer see things you post, start a chat with you or add you as a friend. 
</div>
   <?php
    
}else if(count($profile_post)<1){
    echo "<p class='p-2 bg-white border rounded text-center my-3'>No Posts Yet</p>";
}
        ?>
...
foreach($profile['followers'] as $f){
    ...
    if(checkFollowStatus($f['follower_id'])){
        ...
        $fbtn = '<button class="btn btn-sm btn-success followbtn" data-user-id='.$fuser['id'].' >Follow</button>';
